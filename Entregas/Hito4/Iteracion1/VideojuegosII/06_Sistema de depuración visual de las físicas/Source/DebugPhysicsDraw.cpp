...
/*
* Modificación de este software por DireWolf Games para
* adaptarlo a Lab21
*/
...
void Camera::BuildViewMatrix(float32* m)
{
    // rotamos 180 en eje X   cos=-1   sen=0
	m[0] = 1.0f;
	m[1] = 0.0f;
	m[2] = 0.0f;
	m[3] = 0.0f;
...
//
void DebugPhysicsDraw::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
    if (!m_debuggin)
        return;
...
//
void DebugPhysicsDraw::DrawSolidCircle(const b2Vec2& center, float32 radius, const b2Vec2& axis, const b2Color& color)
{
    if (!m_debuggin)
        return;
...
//
void DebugPhysicsDraw::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color)
{
    if (!m_debuggin)
        return;
...
//
void DebugPhysicsDraw::DrawTransform(const b2Transform& xf)
{
    if (!m_debuggin)
        return;
...
//
void DebugPhysicsDraw::DrawPoint(const b2Vec2& p, float32 size, const b2Color& color)
{
    if (!m_debuggin)
        return;
